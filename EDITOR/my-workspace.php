...
<script>
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                fetch('update-status.php', {
                    ...
                    body: `id=${id}&status=Under Review`
                })
                ...
                .then(data => {
                    if (data.success) {
                        Swal.fire({
                            icon: 'success',
                            title: 'Approved!',
                            text: 'Submission has been moved to "Under Review".',
                        }).then(() => {
    ...
    function updateStatus(id, status) {
        Swal.fire({
            title: `Are you sure to mark this submission as "${status}"?`,
            icon: 'warning',
            showCancelButton: true,
    ...
    // Function to proceed to publication
    function proceedToPublication(id) {
        Swal.fire({
            title: "Are you sure to publish this submission?",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonText: 'Yes',
            cancelButtonText: 'Cancel',
            ...
            if (result.isConfirmed) {
                fetch('publish-submission.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                    body: `id=${id}&action=publish`
                })
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        Swal.fire({
                            icon: 'success',
                            title: 'Success!',
                            text: 'Submission has been published!',
                        }).then(() => {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Oops...',
                            text: data.message || 'Failed to publish submission.',
                        });
                    }
                })
                .catch(error => {
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: 'An error occurred while trying to publish the submission. Please try again later.',
                    });
                });
            } else {
