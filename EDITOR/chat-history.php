...
// Now fetch the usernames of these users along with their last message timestamp and unread status
$user_info = [];
foreach ($chat_users as $chat_user) {
    $chat_user_id = $chat_user['chat_user_id'];
    $last_message_timestamp = $chat_user['last_message_timestamp'];

    // Fetch the username of the chat user
    ...
    if ($user) {
        ...
        // Store the username, last message timestamp, and unread status
        $user_info[] = [
