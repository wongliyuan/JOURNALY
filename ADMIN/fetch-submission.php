...
$stmt = $pdo->prepare("SELECT COUNT(*) AS total FROM submissions WHERE editor_id IS NULL");
$stmt->execute();
$total = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
$totalPages = ceil($total / $limit);

// Fetch submissions for the current page
$stmt = $pdo->prepare("SELECT id, title, category, manuscript_file, DATE_FORMAT(submission_date, '%Y-%m-%d') AS submission_date FROM submissions WHERE editor_id IS NULL LIMIT :limit OFFSET :offset");
