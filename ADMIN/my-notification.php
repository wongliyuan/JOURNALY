          ...
          <div class="notification ${isUnread ? "unread" : ""}">
            <div class="not">
              <a href="#" class="notification-link" 
                ...
                data-new-user="${notification.new_user || ''}">
    ...
    .catch((error) => console.error("Error fetching notifications:", error));
      ...
      notificationLinks.forEach(link => {
        link.addEventListener('click', function (event) {
          ...
          const newUserId = this.dataset.newUser; // Fetch the new user ID if present

          let redirectUrl;

          // Check the content of the notification message
          if (message.includes("new user") && newUserId) {
            redirectUrl = `view-user.php?user_id=${newUserId}`;
          } else {
            redirectUrl = `my-workspace.php`;
          }
